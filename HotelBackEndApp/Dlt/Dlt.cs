...
{
    private static readonly string aesKey = "MySecretAESKey1234";
...
{
    public static string GetRegKeyVal(string argpath= "SOFTWARE\\WOW6432Node\\兴业银锡\\XYAPP")
    {
        // 注册表路径（注意转义反斜杠）
        string registryPath = argpath;// @"SOFTWARE\WOW6432Node\兴业银锡\XYAPP\";
        ...
        try
        {
            ...
            using (RegistryKey key = Registry.LocalMachine.OpenSubKey(registryPath))
            {
                ...
                {
                    // 读取默认值（如果未指定名称）
                    object value = key.GetValue("IP"); // 或者指定具体的值名称，如"IPAddress"
...
public class FuzhangguiOrderRecord
{
    /// <summary>
    /// 消费门店的名称
    /// </summary>
    [Name("消费门店名称")]
    public string ConsumptionStoreName { get; set; }
    ...
    /// </summary>
    [Name("桌台/取餐码")]
    public string TableOrTakeoutCode { get; set; }

    /// <summary>
    /// 开启桌台服务的人员
    /// </summary>
    [Name("开台人")]
    public string TableOpener { get; set; }
...
public class FuzhangguiOrderRecordSum
{
    [Name("消费门店名称")]
    public string FZGConsumptionStoreName { get; set; }
    ...
    [Name("下单时间")]
    public DateTime? FZGOrderPlacedTime { get; set; }

    [Name("结账时间")]
    public DateTime? FZGCheckoutTime { get; set; }

    [Name("完成时间")]
    public DateTime? FZGCompletionTime { get; set; }

    [Name("桌台/取餐码")]
    public string? FZGTableOrPickupCode { get; set; }

    [Name("挂账人")]
    public string? FZGChargeAccountHolder { get; set; }

    [Name("买家微信昵称")]
    public string? FZGBuyerWeChat { get; set; }

    [Name("下单手机号")]
    public string? FZGOrderMobileNumber { get; set; }

    [Name("联系人")]
    public string? FZGContactPerson { get; set; }

    [Name("联系电话")]
    public string? FZGContactPhoneNumber { get; set; }

    [Name("送餐地址")]
    public string? FZGDeliveryAddress { get; set; }

    [Name("会员手机号")]
    public string? FZGMembersMobileNumber { get; set; }

    [Name("会员实体卡号")]
    public string? FZGMembersPhysicalCardNumber { get; set; }

    [Name("会员等级")]
    public string? FZGMemberLevel { get; set; }

    [Name("会员发展门店")]
    public string? FZGMemberRecruitmentStore { get; set; }

    [Name("订单评价")]
    public string? FZGOrderEvaluation { get; set; }

    [Name("单品折让金额")]
    public decimal? FZGUnitDiscountAmount { get; set; }

    [Name("整单折让金额")]
    public decimal? FZGOverallOrderDiscountAmount { get; set; }

    [Name("优惠券抵扣金额")]
    public decimal? FZGCouponDeductionAmount { get; set; }

    [Name("积分抵扣金额")]
    public decimal? FZGPointsDeductionAmount { get; set; }

    [Name("套餐优惠金额")]
    public decimal? FZGMealDiscountAmount { get; set; }

    [Name("外卖优惠金额")]
    public decimal? FZGTakeoutDiscountAmount { get; set; }

    [Name("赠送账户优惠")]
    public decimal? FZGGiftAccountDiscount { get; set; }

    [Name("商品会员价优惠")]
    public decimal? FZGProductMemberPriceDiscount { get; set; }

    [Name("会员等级优惠")]
    public decimal? FZGMemberLevelDiscount { get; set; }

    [Name("会员日优惠金额")]
    public decimal? FZGMembersDayDiscountAmount { get; set; }

    [Name("次卡优惠金额")]
    public decimal? FZGMultiUseCardDiscountAmount { get; set; }
...
public class Dlt
{
   private static string dbinfo = "IZ6svqIQh53qb/lplLfMMnFO43QvjL+zg3tpbNI4ln1HuSn4JzD29AnCb7KlhSSRaXCyeykxH0oVU/4mWnIDEw==";

   private static RegistryKey localKey = RegistryKey.OpenBaseKey(Microsoft.Win32.RegistryHive.CurrentUser, RegistryView.Registry32);

    private static string[] locationIp = RegistryReader.GetRegKeyVal().Split(":");
    ...
    protected static readonly string mysqlConnStr = $"Server={locationIp[0]};Port={locationIp[1]}"+SqlEncryptor.DecryptSql(dbinfo);
    //protected static readonly string mysqlConnStr = $"Server=127.0.0.1;Port=3309;Database=hotel;User=root;Password=123456" ;
    ...
    public int CheckExist(DateTime argstartdate, DateTime argenddate)
    {
        int affectedRows = 0;
        using (var dbConnection = new MySqlConnection(mysqlConnStr))
        {
            string checkQuery = "select count(*) from guestcheck WHERE DATE(openDatetime) >= @date1 and  DATE(openDatetime) <= @date2";
            affectedRows = dbConnection.QueryFirstOrDefault<int>(checkQuery, new { date1 = argstartdate.Date, 
                                                date2 = argenddate.Date });                       
        }
        return affectedRows;
    }
    public void DeleteDataByDate(string dateString, string connectionString)
    {
        ...
        string[] tables = { "guestcheckdetailssumrow", "guestcheckdetails", "guestcheck" , "fuzhanggui" , "fuzhangguiordersum" };
    ...
    public  void SyncData(String argCurrentDateStr = "2025-03-14")
    {
        ...
        string masterQueryE = "";
        ...
        using (var mysqlConn = new MySqlConnection(mysqlConnStr))
        {
            try
            {
                ...
                string masterInsertQuery = @"INSERT INTO guestcheck (guestCheckID, busDate, locationid, revenuecenterid, checkNum, openDateTime, checkTotal, numItems, firstName, lastName,rcsname) 
                VALUES (@guestCheckID, @busDate, @locationid, @revenuecenterid, @checkNum, @openDateTime, @checkTotal, @numItems, @firstName, @lastName,@rcsname);";
...
VALUES  (@transTime, @serviceRoundNum, @lineNum, @guestCheckLineItemID, @detailType,@itemName,@itemName2,
   @itemchname, @rvcName,@firstName,@lastName,@reasonVoidText,@returnText,@recordID,@salesTotal,
...
VALUES  (@organizationID, @checkNum, @tableRef, @openDatetime, @duration,@numGuests,@checkRef,
   @locName, @rvcName,@otName,@firstName,@lastName,@guestCheckID);";
    ...
    public void ImportCsvToMySQL(string csvFilePath, string connectionString)
    {
        ...
        if (string.Equals(Path.GetExtension(csvFilePath), ".csv", StringComparison.OrdinalIgnoreCase))
        {
            if(csvFilePath.Contains("A订单详情总表"))
            {
                CsvConverter.ConvertCsvEncoding(csvFilePath, csvFilePath, false);
            }
            else { 
        ...
        if (csvFilePath.Contains("A订单详情总表"))
        {
            ...
            {
                connection.Open();
                var records2 = new List<FuzhangguiOrderRecordSum>();
                ...
                {
                    int linesToSkip = 4; // 跳过前两行（从第五行开始读取）
                    ...
                    LogHelper.Info("===csv retrieving order data===");
                    records2 = csv.GetRecords<FuzhangguiOrderRecordSum>().ToList();
                    LogHelper.Info($"===csv order data {records2.Count}===");
                    foreach (var record in records2)
                    {
                        LogHelper.Info($"===csv details {record.FZGOrderNumber}===");
                        string query = @"INSERT INTO FuzhangguiOrdersum (
                                FZGOrderNumber,   FZGOrderAmount,   FZGReceivedAmount,   FZGOrderStatus, 
                                FZGPayType,   FZGOrderPlacedTime,   FZGCheckoutTime,  FZGCompletionTime, FZGCasher,
                                FZGRemark,  openDateTime
                            )
                            VALUES (   @FZGOrderNumber,   @FZGOrderAmount,   @FZGReceivedAmount,   
                                    @FZGOrderStatus,   @FZGPayType,   @FZGOrderPlacedTime,   @FZGCheckoutTime, 
                                    @FZGCompletionTime, @FZGCasher,   @FZGRemark,  @FZGOrderPlacedTime
                            )";

                        using (var cmd = new MySqlCommand(query, connection))
                        {
                            cmd.Parameters.AddWithValue("@FZGOrderNumber", record.FZGOrderNumber);
                            cmd.Parameters.AddWithValue("@FZGOrderAmount", record.FZGOrderAmount);
                            cmd.Parameters.AddWithValue("@FZGReceivedAmount", record.FZGReceivedAmount);
                            cmd.Parameters.AddWithValue("@FZGOrderStatus", record.FZGOrderStatus);
                            cmd.Parameters.AddWithValue("@FZGPayType", record.FZGPayType);
                            cmd.Parameters.AddWithValue("@FZGOrderPlacedTime", record.FZGOrderPlacedTime);
                            cmd.Parameters.AddWithValue("@FZGCheckoutTime", record.FZGCheckoutTime);
                            cmd.Parameters.AddWithValue("@FZGCompletionTime", record.FZGCompletionTime);
                            cmd.Parameters.AddWithValue("@FZGCasher", record.FZGCasher);                            
                            cmd.Parameters.AddWithValue("@FZGRemark", record.FZGRemark);
                            if ( cmd.ExecuteNonQuery()>0)  LogHelper.Info($"===csv order details {record.FZGOrderNumber}===");
                            else LogHelper.Info($"===csv order details {record.FZGOrderNumber} insert error===");
                        }

                    }
                }
                connection.Close();
                LogHelper.Info("csv order data uploaded!");
                LogHelper.Info("A订单详情总表导入完成！");
            }
        ...
        {  // 导入商品销售明细..
            using (var connection = new MySqlConnection(connectionString))
            {
                ...
                }))
                {
                    ...
                    foreach (var record in records)
                    {
                        ...
                        string query = @"INSERT INTO fuzhanggui 
                                    ...
                                    orderdate, ordertime, paydate, paytime, price, qty, unit, orgprice, disprice, actualmount, refund, discount, 
                                    channel, pickcode, tableref, realinventory, member, membername, memberlevel, membercardno, orderstaff, opentable, 
                                    itemmemo, ordermemo,openDateTime)
                                    VALUES 
                                    (@storename, @storeid, @itemname, @paymethod, @subitemname, @property, @barcode, @itemgroup, @finitemgroup, @ordernum, 
                                    @orderdate, @ordertime, @paydate, @paytime, @price, @qty, @unit, @orgprice, @disprice, @actualmount, @refund, @discount, 
                                    @channel, @pickcode, @tableref, @realinventory, @member, @membername, @memberlevel, @membercardno, @orderstaff, @opentable, 
                                    @itemmemo, @ordermemo,@paydate)";
                        ...
                        using (var cmd = new MySqlCommand(query, connection))
                        {
                            ...
                            if (cmd.ExecuteNonQuery() > 0) LogHelper.Info($"===csv details {record.Ordernum}===");
                            else LogHelper.Info($"===csv details {record.Ordernum} insert error===");
                        }
                ...
                LogHelper.Info("商品销售明细数据导入完成！");
                //Console.WriteLine("商品销售明细数据导入完成！");
            }
